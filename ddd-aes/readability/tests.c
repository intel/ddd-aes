...
static void test_enc_dec_2blocks()
{
    ...
    int status =
      ddd_aes_ref_read_encrypt(ciphertext, plaintext, 2 * BLOCKSIZE_BYTES, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status < 0) {
        printf("ERROR: ddd_aes_ref_read_encrypt() failed, status: %d\n", status);
        exit(1);
    }

    status = ddd_aes_ref_read_decrypt(decrypted, ciphertext, 2 * BLOCKSIZE_BYTES, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status < 0) {
        printf("ERROR: ddd_aes_ref_read_decrypt() failed, status: %d\n", status);
        exit(1);
    }
...
static void test_enc_dec_3blocks()
{
    ...
    int status =
      ddd_aes_ref_read_encrypt(ciphertext, plaintext, 3 * BLOCKSIZE_BYTES, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status < 0) {
        printf("ERROR: ddd_aes_ref_read_encrypt() failed, status: %d\n", status);
        exit(1);
    }

    status = ddd_aes_ref_read_decrypt(decrypted, ciphertext, 3 * BLOCKSIZE_BYTES, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status < 0) {
        printf("ERROR: ddd_aes_ref_read_decrypt() failed, status: %d\n", status);
        exit(1);
    }
...
static void test_enc_dec_2blocks_andone()
{
    ...
    int status =
      ddd_aes_ref_read_encrypt(ciphertext, plaintext, 2 * BLOCKSIZE_BYTES + 1, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status < 0) {
        printf("ERROR: ddd_aes_ref_read_encrypt() failed, status: %d\n", status);
        exit(1);
    }
    ...
    status =
      ddd_aes_ref_read_decrypt(decrypted, ciphertext, 2 * BLOCKSIZE_BYTES + 1, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status < 0) {
        printf("ERROR: ddd_aes_ref_read_decrypt() failed, status: %d\n", status);
        exit(1);
    }
...
static void test_neg_input_enc()
{
    ...
    int status = ddd_aes_ref_read_encrypt(NULL, plaintext, sizeof(plaintext), key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null ciphertext): ddd_aes_ref_read_encrypt() should have errored out, status: %d\n",
               status);
        exit(1);
    }

    // NULL pointer for plaintext
    status = ddd_aes_ref_read_encrypt(ciphertext, NULL, sizeof(plaintext), key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null plaintext): ddd_aes_ref_read_encrypt() should have errored out, status: %d\n",
               status);
        exit(1);
    }

    // NULL pointer for key_k
    status = ddd_aes_ref_read_encrypt(ciphertext, plaintext, sizeof(plaintext), NULL, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null key_k): ddd_aes_ref_read_encrypt() should have errored out, status: %d\n", status);
        exit(1);
    }

    // NULL pointer for key_l
    status = ddd_aes_ref_read_encrypt(ciphertext, plaintext, sizeof(plaintext), key_k, NULL, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null key_l): ddd_aes_ref_read_encrypt() should have errored out, status: %d\n", status);
        exit(1);
    }

    // NULL pointer for tweak
    status = ddd_aes_ref_read_encrypt(ciphertext, plaintext, sizeof(plaintext), key_k, key_l, NULL, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null tweak): ddd_aes_ref_read_encrypt() should have errored out, status: %d\n", status);
        exit(1);
    }

    // Zero plaintext length
    status = ddd_aes_ref_read_encrypt(ciphertext, plaintext, 0, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (zero plaintext length): ddd_aes_ref_read_encrypt() should have errored out, status: %d\n",
               status);
        exit(1);
    }

    // Too small plaintext length
    status = ddd_aes_ref_read_encrypt(ciphertext, plaintext, 1 * BLOCKSIZE_BYTES, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (too small plaintext length): ddd_aes_ref_read_encrypt() should have errored out, "
               "status: %d\n",
               status);
        exit(1);
    }
    ...
    status = ddd_aes_ref_read_encrypt(
      ciphertext, plaintext, MAXENCRYPTBLOCKS * (size_t)BLOCKSIZE_BYTES, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (too big plaintext length - MAXENCRYPTBLOCKS): ddd_aes_ref_read_encrypt() should have "
               "errored out, "
               "status: %d\n",
               status);
        exit(1);
    ...
    status = ddd_aes_ref_read_encrypt(ciphertext, plaintext, SIZE_MAX, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (too big plaintext length - SIZE_MAX): ddd_aes_ref_read_encrypt() should have "
               "errored out, "
               "status: %d\n",
               status);
        exit(1);
    ...
    // Zero tweak length
    status = ddd_aes_ref_read_encrypt(ciphertext, plaintext, sizeof(plaintext), key_k, key_l, tweak, 0);
    if (status >= 0) {
        printf("TEST FAILED (zero tweak length): ddd_aes_ref_read_encrypt() should have errored out, status: %d\n",
               status);
        exit(1);
    }
    ...
    status =
      ddd_aes_ref_read_encrypt(ciphertext, plaintext, sizeof(plaintext), key_k, key_l, tweak, TWEAKSIZE_BYTES - 1);
    if (status >= 0) {
        printf("TEST FAILED (too small tweak length): ddd_aes_ref_read_encrypt() should have errored out, "
               "status: %d\n",
               status);
        exit(1);
    }
    ...
    status =
      ddd_aes_ref_read_encrypt(ciphertext, plaintext, sizeof(plaintext), key_k, key_l, tweak, TWEAKSIZE_BYTES + 1);
    if (status >= 0) {
        printf("TEST FAILED (too big tweak length - TWEAKSIZE_BYTES + 1): ddd_aes_ref_read_encrypt() should have "
               "errored out, "
               "status: %d\n",
               status);
        exit(1);
    ...
    status = ddd_aes_ref_read_encrypt(ciphertext, plaintext, sizeof(plaintext), key_k, key_l, tweak, SIZE_MAX);
    if (status >= 0) {
        printf("TEST FAILED (too big tweak length - SIZE_MAX): ddd_aes_ref_read_encrypt() should have "
               "errored out, "
               "status: %d\n",
               status);
        exit(1);
...
static void test_neg_input_dec()
{
    ...
    // NULL pointer for ciphertext
    int status = ddd_aes_ref_read_decrypt(plaintext, NULL, sizeof(ciphertext), key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null ciphertext): ddd_aes_ref_read_decrypt() should have errored out, status: %d\n",
               status);
        exit(1);
    }

    // NULL pointer for plaintext
    status = ddd_aes_ref_read_decrypt(NULL, ciphertext, sizeof(ciphertext), key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null plaintext): ddd_aes_ref_read_decrypt() should have errored out, status: %d\n",
               status);
        exit(1);
    }

    // NULL pointer for key_k
    status = ddd_aes_ref_read_decrypt(plaintext, ciphertext, sizeof(ciphertext), NULL, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null key_k): ddd_aes_ref_read_decrypt() should have errored out, status: %d\n", status);
        exit(1);
    }

    // NULL pointer for key_l
    status = ddd_aes_ref_read_decrypt(plaintext, ciphertext, sizeof(ciphertext), key_k, NULL, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null key_l): ddd_aes_ref_read_decrypt() should have errored out, status: %d\n", status);
        exit(1);
    }

    // NULL pointer for tweak
    status = ddd_aes_ref_read_decrypt(plaintext, ciphertext, sizeof(ciphertext), key_k, key_l, NULL, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (null tweak): ddd_aes_ref_read_decrypt() should have errored out, status: %d\n", status);
        exit(1);
    }

    // Zero ciphertext length
    status = ddd_aes_ref_read_decrypt(plaintext, ciphertext, 0, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (zero ciphertext length): ddd_aes_ref_read_decrypt() should have errored out, status: %d\n",
               status);
        exit(1);
    }

    // Too small ciphertext length
    status = ddd_aes_ref_read_decrypt(plaintext, ciphertext, 1 * BLOCKSIZE_BYTES, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (too small ciphertext length): ddd_aes_ref_read_decrypt() should have errored out, "
               "status: %d\n",
               status);
        exit(1);
    }
    ...
    status = ddd_aes_ref_read_decrypt(
      plaintext, ciphertext, MAXENCRYPTBLOCKS * (size_t)BLOCKSIZE_BYTES, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (too big ciphertext length - MAXENCRYPTBLOCKS): ddd_aes_ref_read_decrypt() should have "
               "errored out, "
               "status: %d\n",
               status);
        exit(1);
    ...
    status = ddd_aes_ref_read_decrypt(plaintext, ciphertext, SIZE_MAX, key_k, key_l, tweak, TWEAKSIZE_BYTES);
    if (status >= 0) {
        printf("TEST FAILED (too big ciphertext length - SIZE_MAX): ddd_aes_ref_read_decrypt() should have "
               "errored out, "
               "status: %d\n",
               status);
        exit(1);
    ...
    // Zero tweak length
    status = ddd_aes_ref_read_decrypt(plaintext, ciphertext, sizeof(ciphertext), key_k, key_l, tweak, 0);
    if (status >= 0) {
        printf("TEST FAILED (zero tweak length): ddd_aes_ref_read_decrypt() should have errored out, status: %d\n",
               status);
        exit(1);
    }
    ...
    // Too small tweak length
    status =
      ddd_aes_ref_read_decrypt(plaintext, ciphertext, sizeof(ciphertext), key_k, key_l, tweak, TWEAKSIZE_BYTES - 1);
    if (status >= 0) {
        printf("TEST FAILED (too small tweak length): ddd_aes_ref_read_decrypt() should have errored out, "
               "status: %d\n",
               status);
        exit(1);
    }
    ...
    status =
      ddd_aes_ref_read_decrypt(plaintext, ciphertext, sizeof(ciphertext), key_k, key_l, tweak, TWEAKSIZE_BYTES + 1);
    if (status >= 0) {
        printf("TEST FAILED (too big tweak length - TWEAKSIZE_BYTES + 1): ddd_aes_ref_read_decrypt() should have "
               "errored out, "
               "status: %d\n",
               status);
        exit(1);
    ...
    // Too big tweak length (SIZE_MAX)
    status = ddd_aes_ref_read_decrypt(plaintext, ciphertext, sizeof(ciphertext), key_k, key_l, tweak, SIZE_MAX);
    if (status >= 0) {
        printf("TEST FAILED (too big tweak length - SIZE_MAX): ddd_aes_ref_read_decrypt() should have "
               "errored out, "
               "status: %d\n",
               status);
        exit(1);
