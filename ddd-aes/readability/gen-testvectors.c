...
int main(int argc, char* argv[])
{
    ...
    for (uint32_t i = 2 * BLOCKSIZE_BYTES; i <= MAXTESTSIZE; i += TESTINCREMENT) {
        ...
        int status = ddd_aes_ref_read_encrypt(ciphertext, plaintext, i, key_k, key_l, tweak, TWEAKSIZE_BYTES);
        if (status < 0) {
            printf("ERROR: ddd_aes_ref_read_encrypt() failed, status: %d\n", status);
            return 1;
        }

        status = ddd_aes_ref_read_decrypt(decrypted, ciphertext, i, key_k, key_l, tweak, TWEAKSIZE_BYTES);
        if (status < 0) {
