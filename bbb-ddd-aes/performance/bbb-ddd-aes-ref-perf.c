...
#include "bbb-ddd-aes-ref-perf.h"
...
void precompute_key(const uint8_t* const key, uint8_t* expanded_key)
{
    ...
    precompute_aes_key(key + AES_KEYSIZE_BYTES, expanded_key + 176);
    for (size_t i = 0; i < POLYVAL_KEYSIZE_BYTES; i++) {
        expanded_key[(2 * 176) + i] = key[(2 * AES_KEYSIZE_BYTES) + i];
    }

    __m128i hk = _mm_loadu_si128((__m128i*)(expanded_key + ((2 * 176))));
    __m128i hk1;
    __m128i hk2;
    __m128i hk3;
    ...
    _mm_storeu_si128((__m128i*)(expanded_key + (2 * 176) + POLYVAL_KEYSIZE_BYTES), hk1);
    _mm_storeu_si128((__m128i*)(expanded_key + (2 * 176) + (2 * POLYVAL_KEYSIZE_BYTES)), hk2);
    _mm_storeu_si128((__m128i*)(expanded_key + (2 * 176) + (3 * POLYVAL_KEYSIZE_BYTES)), hk3);
...
/// @brief BBB-DDD-AES encryption function core implementation.
/// @param[in] m Plaintext (input).
/// @param[out] c Ciphertext (output). The result of BBB-DDD-AES encryption function application to @p m.
/// @param[in] blen Plaintext length (in bytes).
/// @param[in] k BBB-DDD-AES key (expanded form).
/// @param[in] t Tweak.
                                            ...
                                            const uint8_t* const t)
{
    ...
    uint64_t ALIGN128 help[2];
    help[0] = (0x2ULL << 0) | ((uint64_t)t[0]) << 4 | ((uint64_t)t[1]) << 12 | ((uint64_t)t[2]) << 20 |
              ((uint64_t)t[3]) << 28 | ((uint64_t)t[4]) << 36 | ((uint64_t)t[5]) << 44 | ((uint64_t)t[6]) << 52 |
              ((uint64_t)t[7]) << 60;
    help[1] = ((uint64_t)t[7]) >> 4 | ((uint64_t)t[8]) << 4 | ((uint64_t)t[9]) << 12 | ((uint64_t)t[10]) << 20 |
              ((uint64_t)t[11]) << 28;
    __m128i mask = _mm_loadu_si128((__m128i*)help);

    __m128i hk = _mm_loadu_si128((__m128i*)(k + ((2 * 176))));
    __m128i hk1;
    __m128i hk2;
    __m128i hk3;

    if (blen_full_4_blocks > 16) {
        hk1 = _mm_loadu_si128((__m128i*)(k + (2 * 176) + POLYVAL_KEYSIZE_BYTES));
        hk2 = _mm_loadu_si128((__m128i*)(k + (2 * 176) + (2 * POLYVAL_KEYSIZE_BYTES)));
        hk3 = _mm_loadu_si128((__m128i*)(k + (2 * 176) + (3 * POLYVAL_KEYSIZE_BYTES)));
    }

    help[1] = 1ULL << 36;
    help[0] = 0;
    const __m128i one = _mm_loadu_si128((__m128i*)(help));
    ...
    for (uint32_t i = BLOCKSIZE_BYTES; i < blen_full_4_blocks; i += (4 * BLOCKSIZE_BYTES)) {
        ...
        mask = _mm_add_epi64(mask, one);
        __m128i wmask4 = mask;
        wmask1 = _mm_xor_si128(wmask1, ((__m128i*)k)[11]);
        wmask2 = _mm_xor_si128(wmask2, ((__m128i*)k)[11]);
        wmask3 = _mm_xor_si128(wmask3, ((__m128i*)k)[11]);
        wmask4 = _mm_xor_si128(wmask4, ((__m128i*)k)[11]);
        for (uint32_t j = 12; j < 21; j++) {
        ...
        wmask1 = _mm_aesenclast_si128(wmask1, ((__m128i*)k)[21]);
        wmask2 = _mm_aesenclast_si128(wmask2, ((__m128i*)k)[21]);
        wmask3 = _mm_aesenclast_si128(wmask3, ((__m128i*)k)[21]);
        wmask4 = _mm_aesenclast_si128(wmask4, ((__m128i*)k)[21]);
        _mm_storeu_si128((__m128i*)(buffer + i - BLOCKSIZE_BYTES), wmask1);
        _mm_storeu_si128((__m128i*)(buffer + i), wmask2);
        _mm_storeu_si128((__m128i*)(buffer + i + BLOCKSIZE_BYTES), wmask3);
        _mm_storeu_si128((__m128i*)(buffer + i + (2 * BLOCKSIZE_BYTES)), wmask4);
        // Load state
    ...
    for (uint32_t i = blen_full_4_blocks; i < blen_full_blocks; i += BLOCKSIZE_BYTES) {
        ...
        mask = _mm_add_epi64(mask, one);
        __m128i wmask = mask;
        wmask = _mm_xor_si128(wmask, ((__m128i*)k)[11]);
        for (uint32_t j = 12; j < 21; j++) {
            wmask = _mm_aesenc_si128(wmask, ((__m128i*)k)[j]);
        }
        wmask = _mm_aesenclast_si128(wmask, ((__m128i*)k)[21]);
        _mm_storeu_si128((__m128i*)(buffer + i - BLOCKSIZE_BYTES), wmask);
        // Load state
        ...
        zeroize_secret(&wmask, sizeof(wmask));
    ...
    if (blen_full_blocks != blen) {
        ...
        wmask = _mm_aesenclast_si128(wmask, ((__m128i*)k)[21]);
        _mm_storeu_si128((__m128i*)(buffer + blen_full_blocks - BLOCKSIZE_BYTES), wmask);
        // First universal hash
        hval = _mm_xor_si128(hval, state);
        hval = polyval_dot(hval, hk);
    ...
    k0 = _mm_loadu_si128(((__m128i*)k) + 11);
    k1 = _mm_loadu_si128(((__m128i*)k) + 12);
    k2 = _mm_loadu_si128(((__m128i*)k) + 13);
    k3 = _mm_loadu_si128(((__m128i*)k) + 14);
    k4 = _mm_loadu_si128(((__m128i*)k) + 15);
    k5 = _mm_loadu_si128(((__m128i*)k) + 16);
    k6 = _mm_loadu_si128(((__m128i*)k) + 17);
    k7 = _mm_loadu_si128(((__m128i*)k) + 18);
    k8 = _mm_loadu_si128(((__m128i*)k) + 19);
    k9 = _mm_loadu_si128(((__m128i*)k) + 20);
    k10 = _mm_loadu_si128(((__m128i*)k) + 21);
    ...
    // Mask for the first PRF and all secondary masks
    uint64_t mmask1_1[2], mmask1_a[2], mmask2_a[2];
    mmask1_1[0] = mmask1_a[0] = (0x1ULL << 0) | ((uint64_t)t[0]) << 4 | ((uint64_t)t[1]) << 12 |
                                ((uint64_t)t[2]) << 20 | ((uint64_t)t[3]) << 28 | ((uint64_t)t[4]) << 36 |
                                ((uint64_t)t[5]) << 44 | ((uint64_t)t[6]) << 52 | ((uint64_t)t[7]) << 60;
    mmask2_a[0] = (0x2ULL << 0) | ((uint64_t)t[0]) << 4 | ((uint64_t)t[1]) << 12 | ((uint64_t)t[2]) << 20 |
                  ((uint64_t)t[3]) << 28 | ((uint64_t)t[4]) << 36 | ((uint64_t)t[5]) << 44 | ((uint64_t)t[6]) << 52 |
                  ((uint64_t)t[7]) << 60;
    mmask1_a[1] = mmask1_1[1] = mmask2_a[1] = ((uint64_t)t[7]) >> 4 | ((uint64_t)t[8]) << 4 | ((uint64_t)t[9]) << 12 |
                                              ((uint64_t)t[10]) << 20 | ((uint64_t)t[11]) << 28;
    mmask1_1[1] += 1ULL << 36;

    __m128i mask1_1 = _mm_loadu_si128((__m128i*)mmask1_1);
    __m128i mask1_a = _mm_loadu_si128((__m128i*)mmask1_a);
    __m128i mask2_a = _mm_loadu_si128((__m128i*)mmask2_a);

    mask1_1 = _mm_xor_si128(mask1_1, k0);
    mask1_a = _mm_xor_si128(mask1_a, k0);
    mask2_a = _mm_xor_si128(mask2_a, k0);
    mask1_1 = _mm_aesenc_si128(mask1_1, k1);
    mask1_a = _mm_aesenc_si128(mask1_a, k1);
    mask2_a = _mm_aesenc_si128(mask2_a, k1);
    mask1_1 = _mm_aesenc_si128(mask1_1, k2);
    mask1_a = _mm_aesenc_si128(mask1_a, k2);
    mask2_a = _mm_aesenc_si128(mask2_a, k2);
    mask1_1 = _mm_aesenc_si128(mask1_1, k3);
    mask1_a = _mm_aesenc_si128(mask1_a, k3);
    mask2_a = _mm_aesenc_si128(mask2_a, k3);
    mask1_1 = _mm_aesenc_si128(mask1_1, k4);
    mask1_a = _mm_aesenc_si128(mask1_a, k4);
    mask2_a = _mm_aesenc_si128(mask2_a, k4);
    mask1_1 = _mm_aesenc_si128(mask1_1, k5);
    mask1_a = _mm_aesenc_si128(mask1_a, k5);
    mask2_a = _mm_aesenc_si128(mask2_a, k5);
    mask1_1 = _mm_aesenc_si128(mask1_1, k6);
    mask1_a = _mm_aesenc_si128(mask1_a, k6);
    mask2_a = _mm_aesenc_si128(mask2_a, k6);
    mask1_1 = _mm_aesenc_si128(mask1_1, k7);
    mask1_a = _mm_aesenc_si128(mask1_a, k7);
    mask2_a = _mm_aesenc_si128(mask2_a, k7);
    mask1_1 = _mm_aesenc_si128(mask1_1, k8);
    mask1_a = _mm_aesenc_si128(mask1_a, k8);
    mask2_a = _mm_aesenc_si128(mask2_a, k8);
    mask1_1 = _mm_aesenc_si128(mask1_1, k9);
    mask1_a = _mm_aesenc_si128(mask1_a, k9);
    mask2_a = _mm_aesenc_si128(mask2_a, k9);
    mask1_1 = _mm_aesenclast_si128(mask1_1, k10);
    mask1_a = _mm_aesenclast_si128(mask1_a, k10);
    mask2_a = _mm_aesenclast_si128(mask2_a, k10);
    ...
    // First PRF
    mask1_1 = _mm_xor_si128(hval, mask1_1);
    mask1_a = _mm_xor_si128(hval, mask1_a);
    ...
    hval = _mm_xor_si128(hval, mask1_a);
    _mm_storeu_si128((__m128i*)(c + blen - BLOCKSIZE_BYTES), hval);

    // Second PRF
    mask2_a = _mm_xor_si128(mask2_a, hval);
    mask2_a = _mm_xor_si128(mask2_a, k0);
    mask2_a = _mm_aesenc_si128(mask2_a, k1);
    mask2_a = _mm_aesenc_si128(mask2_a, k2);
    mask2_a = _mm_aesenc_si128(mask2_a, k3);
    ...
    for (uint32_t i = 0; i < blen_full_8_blocks - BLOCKSIZE_BYTES; i += (8 * BLOCKSIZE_BYTES)) {
        ...
        wmask1 = _mm_xor_si128(wmask1, mask);
        wmask2 = _mm_xor_si128(wmask2, mask);
        wmask3 = _mm_xor_si128(wmask3, mask);
        wmask4 = _mm_xor_si128(wmask4, mask);
        ...
        wmask1 = _mm_xor_si128(wmask1, mask2_a);
        wmask2 = _mm_xor_si128(wmask2, state2);
        wmask2 = _mm_xor_si128(wmask2, mask2_a);
        wmask3 = _mm_xor_si128(wmask3, state3);
        wmask3 = _mm_xor_si128(wmask3, mask2_a);
        wmask4 = _mm_xor_si128(wmask4, state4);
        wmask4 = _mm_xor_si128(wmask4, mask2_a);
        ...
        wmask5 = _mm_xor_si128(wmask5, mask);
        wmask6 = _mm_xor_si128(wmask6, mask);
        wmask7 = _mm_xor_si128(wmask7, mask);
        wmask8 = _mm_xor_si128(wmask8, mask);
        ...
        wmask5 = _mm_xor_si128(wmask5, mask2_a);
        wmask6 = _mm_xor_si128(wmask6, state6);
        wmask6 = _mm_xor_si128(wmask6, mask2_a);
        wmask7 = _mm_xor_si128(wmask7, state7);
        wmask7 = _mm_xor_si128(wmask7, mask2_a);
        wmask8 = _mm_xor_si128(wmask8, state8);
        wmask8 = _mm_xor_si128(wmask8, mask2_a);
    ...
    for (uint32_t i = blen_full_8_blocks - BLOCKSIZE_BYTES; i < blen_full_4_blocks - BLOCKSIZE_BYTES;
        ...
        __m128i wmask2 = _mm_loadu_si128((__m128i*)(buffer + i + BLOCKSIZE_BYTES));
    ...
    zeroize_secret(&mask, sizeof(mask));
    ...
    zeroize_secret(&mask1_a, sizeof(mask1_a));
    zeroize_secret(&mask2_a, sizeof(mask2_a));

    return 0;
}

/// @brief BBB-DDD-AES decryption function core implementation.
/// @param[in] c Ciphertext (input).
/// @param[out] m Plaintext (output). The result of BBB-DDD-AES decryption function application to @p c.
                                            ...
                                            const uint8_t* const t)
{
    ...
    for (uint32_t i = 0; i < blen_full_blocks - BLOCKSIZE_BYTES; i += BLOCKSIZE_BYTES) {
        ...
        mask = _mm_add_epi64(mask, one);
        __m128i wmask = mask;
        wmask = _mm_xor_si128(wmask, k0);
        wmask = _mm_aesenc_si128(wmask, k1);
        wmask = _mm_aesenc_si128(wmask, k2);
        wmask = _mm_aesenc_si128(wmask, k3);
        wmask = _mm_aesenc_si128(wmask, k4);
        wmask = _mm_aesenc_si128(wmask, k5);
        wmask = _mm_aesenc_si128(wmask, k6);
        wmask = _mm_aesenc_si128(wmask, k7);
        wmask = _mm_aesenc_si128(wmask, k8);
        wmask = _mm_aesenc_si128(wmask, k9);
        wmask = _mm_aesenclast_si128(wmask, k10);
        _mm_storeu_si128((__m128i*)(buffer + i), wmask);
        // Second universal hash
        hval = _mm_xor_si128(hval, state);
        hval = polyval_dot(hval, hk);
