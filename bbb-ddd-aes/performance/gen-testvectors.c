...
int main(int argc, char* argv[])
{
    ...
    for (uint32_t i = 2 * 16; i <= MAXTESTSIZE; i += TESTINCREMENT) {
        ...
        int status = bbb_ddd_aes_ref_perf_encrypt(ciphertext, plaintext, i, ek, tweak, TWEAKSIZE_BYTES);
        if (status < 0) {
            printf("ERROR: bbb_ddd_aes_ref_perf_encrypt() failed, status: %d\n", status);
            return 1;
        }

        status = bbb_ddd_aes_ref_perf_decrypt(decrypted, ciphertext, i, ek, tweak, TWEAKSIZE_BYTES);
        if (status < 0) {
